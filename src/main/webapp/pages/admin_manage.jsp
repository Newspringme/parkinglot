...
<html>
	...
	<body>
		...
		<div id="addAdmin" onsubmit="return false;" style="display: none">
			<form class="layui-form" id="addForm" action="" style="margin-top: 30px">
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block" style="float: left; margin-left: 20px;width: 200px;">
						<input type="radio" checked name="adminSex" value="男" title="男" autocomplete="off"
						       class="layui-input">
		...
		<div style="display: none" id="adminBar">
			...
			<button type="button" id="uploadImg" class="layui-btn layui-btn-sm layui-icon layui-icon-upload-circle"
			        lay-event="uploadImg">上传头像
			</button>

			<%--	<button type="button" class="layui-btn layui-btn-sm layui-icon layui-icon-download-circle" lay-event="queryImg">查看头像</button>--%>
			<button type="button" class="layui-btn layui-btn-sm  layui-btn layui-icon layui-icon-refresh-3"
			        lay-event="return">重置密码
			</button>
			<button type="button" class="layui-btn layui-btn-sm  layui-btn-danger layui-icon layui-icon-more"
			        lay-event="look">查看
			</button>
			{{# if(d.adminState === '启用'){ }}
			<button type="button" class="layui-btn layui-btn-sm  layui-btn-danger layui-icon layui-icon-close"
			        lay-event="changeState">禁用
			</button>
			{{# } }}
			{{# if(d.adminState === '禁用'){ }}
			<button type="button" class="layui-btn layui-btn-sm  layui-btn layui-icon layui-icon-ok"
			        lay-event="changeState">启用
			</button>
			{{# } }}
		</div>
		<!--        上传弹出层-->
		...
		<div id="lookAdmin" onsubmit="return false;" style="display: none; ">
			...
			<div class="layui-form-item">
				<label class="layui-form-label" style="margin-left: 120px;">账号名称：</label>
				<div class="layui-input-block" style="float: left; margin-left: 20px;width: 200px;">
					<input type="text" name="adminName1" id="adminName1" lay-verify="adminName"
			...
			<div class="layui-form-item">
				<label class="layui-form-label" style="margin-left: 120px;">性&emsp;&emsp;别：</label>
				<div class="layui-input-block" style="float: left; margin-left: 20px;width: 200px;">
					<input type="text" name="adminName2" id="adminName2" lay-verify="adminSex"
					       autocomplete="off" class="layui-input" disabled>
				</div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label" style="margin-left: 120px;">手机号码：</label>
				<div class="layui-input-block" style="float: left; margin-left: 20px;width: 200px;">
					<input type="tel" name="adminName3" id="adminName3" autocomplete="off" class="layui-input "
					       disabled>
				</div>
			</div>

			<input type="hidden" name="adminId"
			       autocomplete="off" class="layui-input layui-form-item adminId" disabled>

			<div class="layui-form-item">
				<label class="layui-form-label" style="margin-left: 120px;">所属角色：</label>
				<div class="layui-input-block" style="float: left; margin-left: 20px;width: 200px;">
					<input type="tel" name="adminName4" id="adminName4" autocomplete="off" class="layui-input "
					       disabled>
				</div>
			</div>
			...
			layui.use(['jquery', 'layer', 'form', 'table', 'laydate', 'upload'], function () {
				...
				table.on('tool(adminTable)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent == 'uploadImg') { //上传头像
						...
						openUpdateAdmin(obj);
					} else if (layEvent === 'changeState') {//更新状态
						alert('更新管理员状态');
						console.log(obj.data);
						var state = obj.data.adminState;
						var newState;
						if (state === '启用') {
							newState = "禁用";
						} else {
						...
						layer.confirm('是否' + newState + '？', {icon: 7}, function (index) {
							layer.close(index);
							$.post("${pageContext.request.contextPath}/AdminController/updateState?adminName=" + data.adminName + "&adminState=" + newState, null, function (msg) {
								msg = msg + '';
								if (msg == 'true') {
						...
						$('#adminName1').val(obj.data.adminName);
						$('#adminName2').val(obj.data.adminSex);
						$('#adminName3').val(obj.data.adminTel);
						$('#adminName4').val(obj.data.roleName);
					}
